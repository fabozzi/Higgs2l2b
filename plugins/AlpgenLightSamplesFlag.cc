...
#include "HiggsAnalysis/Higgs2l2b/interface/AlpgenLightSamplesFlag.hh"

//-------------------------------------------------------
//-------------------------------------------------------


AlpgenLightSamplesFlag::AlpgenLightSamplesFlag(const edm::ParameterSet& iConfig)
{
...
AlpgenLightSamplesFlag::~AlpgenLightSamplesFlag()
{
}

//-------------------------------------------------------

void AlpgenLightSamplesFlag::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
...
//-------------------------------------------------------
//-------------------------------------------------------
bool AlpgenLightSamplesFlag::processCCPair (GenParticleCollection genpart)
...
//----------------------------------------------------------------------
bool AlpgenLightSamplesFlag::processBBPair (GenParticleCollection genpart)
...
//----------------------------------------------------------------------
bool AlpgenLightSamplesFlag::processSingleC (GenParticleCollection genpart)
...
//-------------------------------------------------------
//-------------------------------------------------------
bool AlpgenLightSamplesFlag::cuttingSingleC (float pt)
  // Makes the selection for single-C events, using the pt of the charm quark.
  // Returns true if the event is rejected.
{
  if (pt<getSingleCPtCut()) return false; // Events is fine for light samples
  return true; // Events must come from W+C+Np
}

//----------------------------------------------------------------------
//----------------------------------------------------------------------
bool AlpgenLightSamplesFlag::cuttingCCPair (float dr)
  // Makes the selection for CC-pair events, using the maximum of the minimum
  // Dr for the valid pairs.
  // Returns true if the event is rejected.
{
  if (dr<getPairDeltaRCut()) return false; // Events is fine for light samples
  return true; // Events must come from W+CC+Np
}

//----------------------------------------------------------------------
//----------------------------------------------------------------------
bool AlpgenLightSamplesFlag::cuttingBBPair (float dr)
  // Makes the selection for BB-pair events, using the maximum of the minimum
  // Dr for the valid pairs.
  // Returns true if the event is rejected.
{
  if (dr<getPairDeltaRCut()) return false; // Events is fine for light samples
  return true; // Events must come from W+BB+Np
}
//-------------------------------------------------------
//-------------------------------------------------------
void AlpgenLightSamplesFlag::beginJob() {
}

void AlpgenLightSamplesFlag::endJob() {
}

DEFINE_FWK_MODULE(AlpgenLightSamplesFlag);
