...
#include <iostream>
#include <iomanip>
#include "HiggsAnalysis/Higgs2l2b/interface/TKinFitter.h"
#include "HiggsAnalysis/Higgs2l2b/interface/TAbsFitParticle.h"
#include "HiggsAnalysis/Higgs2l2b/interface/TAbsFitConstraint.h"
...
void TKinFitter::setCovMatrix( TMatrixD &V ) {
  ...
  if ( (V.GetNrows() != _nParB) || (V.GetNcols() != _nParB) ) {
    std::cout << "Covariance matrix of measured particles needs to be a " << _nParB << "x" << _nParB << " matrix." << std::endl;
  } else {
    _V.ResizeTo( V );
    _V = V;
  }
...
void TKinFitter::printMatrix(const TMatrixD &matrix, const TString name) {
  ...
  if (!matrix.IsValid()) {
    std::cout << "Matrix " << name << " is invalid.";
    return;
  }
