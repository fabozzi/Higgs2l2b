...
#include "HiggsAnalysis/Higgs2l2b/interface/AlpgenCCSamplesFlag.hh"

//-------------------------------------------------------
//-------------------------------------------------------


AlpgenCCSamplesFlag::AlpgenCCSamplesFlag(const edm::ParameterSet& iConfig)
{
...
AlpgenCCSamplesFlag::~AlpgenCCSamplesFlag()
{
}

//-------------------------------------------------------

void AlpgenCCSamplesFlag::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
...
//-------------------------------------------------------
//-------------------------------------------------------
bool AlpgenCCSamplesFlag::processCCPair (GenParticleCollection genpart)
...
//----------------------------------------------------------------------
bool AlpgenCCSamplesFlag::processBBPair (GenParticleCollection genpart)
...
//----------------------------------------------------------------------
bool AlpgenCCSamplesFlag::processSingleC (GenParticleCollection genpart)
...
//-------------------------------------------------------
//-------------------------------------------------------
bool AlpgenCCSamplesFlag::cuttingSingleC (float)
...
{
  return true; // Events must come from light-flavour or W+C samples
}

//----------------------------------------------------------------------
//----------------------------------------------------------------------
bool AlpgenCCSamplesFlag::cuttingCCPair (float dr)
...
{
  if (dr>=getPairDeltaRCut()) return false; // Event is fine for W+CC samples
  return true; // Event must come from other samples!
}

//----------------------------------------------------------------------
//----------------------------------------------------------------------
bool AlpgenCCSamplesFlag::cuttingBBPair (float dr)
  // Makes the selection for BB-pair events, using the maximum of the minimum
  // Dr for the valid pairs.
  // Returns true if the event is rejected.
{
  if (dr<getPairDeltaRCut()) return false; // Events is fine for W+CC samples
  return true; // Events must come from W+BB+Np
}

//-------------------------------------------------------
//-------------------------------------------------------
void AlpgenCCSamplesFlag::beginJob() {
}

void AlpgenCCSamplesFlag::endJob() {
}

DEFINE_FWK_MODULE(AlpgenCCSamplesFlag);
